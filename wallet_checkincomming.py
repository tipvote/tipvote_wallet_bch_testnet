...
from wallet_getnewaddress import getnewaddress
...
def addtounconfirmed(amount, user_id, txid):
    ...
    unconfirmedtable = BchUnconfirmedTest.query.filter_by(user_id=user_id).first()
    ...
    if unconfirmedtable is None:
        ...
        newunconfirmed = BchUnconfirmedTest(
            user_id=user_id,
            unconfirmed1=0,
            unconfirmed2=0,
            unconfirmed3=0,
...
def removeunconfirmed(user_id, txid):
    ...
    unconfirmeddelete = BchUnconfirmedTest.query.filter_by(user_id=user_id).first()
...
def getbalanceunconfirmed(user_id):
    ...
    wallet = BchWalletTest.query.filter_by(user_id=user_id).first()
...
def orphan(txid, amount2, address):
    ...
    """
    getorphan = BchTransOrphanTest.query.filter_by(txid=txid).first()
    if getorphan:
        pass
    else:
        # orphan transaction..put in background.
        # they prolly sent to old address
        trans = BchTransOrphanTest(
            btc=amount2,
            btcaddress=address,
            txid=txid,
        )
...
def addcoin():
    ...
    if response_json["error"] is not None:
        ...
        for i, j in enumerate(response_json):

            # if response
            if j:

                # get the variables from an array
                address = response_json[i]['address']
                amount = response_json[i]['amount']
                txid = response_json[i]['txid']
                confirmations = response_json[i]['confirmations']
                ...
                userwallets = BchWalletTest.query \
                    .filter(or_(BchWalletTest.address1 == address,
                                BchWalletTest.address2 == address,
                                BchWalletTest.address3 == address
                                )
                            ) \
                    .first()
                ...
                if userwallets:
                    ...
                    transactions = TransactionsBchTest.query\
                        .filter(TransactionsBchTest.txid == txid)\
                        .first()
